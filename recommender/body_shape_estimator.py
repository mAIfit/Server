...
import os
os.environ["PYOPENGL_PLATFORM"] = "osmesa" # set osmesa as render backend for use in SSH
...
class BodyShapeEstimator:
    def __init__(self):
        ...
        os.chdir('/home/myungjune/projects/multiperson')
        ...
        self.yolo, self.pose_estimator, self.ik_net, self.feature_extractor, self.smpl_layer, self.smplTR \
    ...
    def estimate(self, imgfile, betas_only=False):
        ...
        split_images_folder, pose_results_folder, mesh_results_folder \
            = make_folder(self.demo_cfg, imgfile)
        
        orig_img = cv2.imread(imgfile)
        orig_height, orig_width = orig_img.shape[:2]
        renderer = Renderer(smpl=self.smpl_layer, resolution=(orig_width, orig_height), orig_img=True)
        ...
        for person_id, img_patch in enumerate(img_patch_list[:num_person]):
            img_plot, img_pe_input, intrinsic = get_pose_estimator_input(img_patch, self.FLAGS)
            
            with torch.no_grad():
                j2d, j3d, j3d_abs, skeleton_indices, edges \
                    = self.pose_estimator(img_pe_input, intrinsic, intrinsic)
        ...
        axis_angle, rot6d, betas, cam, verts, faces \
            = process_output(self.smpl_layer, refined_rot6d, refined_betas, refined_cam, rotation_angle=angle, rotation_axis=axis)
        ...
        meshed_image_name = save_mesh_rendering(
            ...
            rotation_angle=angle, rotation_axis=axis)

        os.chdir(old_cwd)
        return {
            "betas": betas_ik_net[0].tolist(),
            "meshed_image": meshed_image_name
        }
...
if __name__ == "__main__":
    img_path = '/home/myungjune/projects/multiperson/demo_image/6.jpg'
    assert os.path.exists(img_path)

    bse = BodyShapeEstimator()
    betas, mesh_path =  bse.extract_betas(img_path), bse.extract_mesh(img_path)
    print(betas)
    print(mesh_path)